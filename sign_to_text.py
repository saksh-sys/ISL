...
def main():
    ...
    # Read-only text area
    st.text_area("📄 Recognized Text:", value=st.session_state.detected_text, height=100, disabled=True)
    ...
    while cap.isOpened():
        ...
        # Streamlit does not support real-time loops, so we use a stop button
        if st.button("❌ Stop Camera"):
            break

    cap.release()  # ✅ Fix: Release webcam properly
